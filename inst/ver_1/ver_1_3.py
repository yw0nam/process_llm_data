...
class preprocess(preprocess):
    ...
    def PJMixers_hieunguyenminh_roleplay_deduped_ShareGPT(self):
        ...
        return data
    # def mahiatlinux_Reflection_Dataset_v2(self):
    #     data = pd.DataFrame(datasets.load_dataset('mahiatlinux/Reflection-Dataset-ShareGPT-v2', split='train'))
    #     def to_chat_template(x):
    #         out_list = []
    #         for each_line in x:
    #             if each_line['from'] == 'human':
    ...
    #             return out_list
    #     data['chat_template'] = data['conversations'].map(lambda x: to_chat_template(x))
    #     data['source'] = 'mahiatlinux_Reflection_Dataset_v2'
    #     return data
    ...
    def process_datasets(self) -> dict[pd.DataFrame]:
        ...
        # dicts['mahiatlinux_Reflection_Dataset_v2'] = self.mahiatlinux_Reflection_Dataset_v2()
